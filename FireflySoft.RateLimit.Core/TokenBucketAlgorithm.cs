...
namespace FireflySoft.RateLimit.Core
{
    ...
    public class TokenBucketAlgorithm<TRequest> : UpdatableRateLimitAlgorithm<TRequest>
    {
        ...
        protected override bool CheckSingleRule(string target, IRateLimitStorage storage, RateLimitRule<TRequest> rule)
        {
            ...
            var result = storage.TokenBucketDecrement(target, 1, currentRule.Capacity, (int)currentRule.InflowUnit.TotalMilliseconds, currentRule.InflowQuantityPerUnit, currentRule.StartTimeType, currentRule.LockSeconds);
        ...
        {
            var currentRule = rule as TokenBucketRateLimitRule<TRequest>;
            var result = await storage.TokenBucketDecrementAsync(target, 1, currentRule.Capacity, (int)currentRule.InflowUnit.TotalMilliseconds, currentRule.InflowQuantityPerUnit, currentRule.StartTimeType, currentRule.LockSeconds);
            Debug.WriteLine("check result:" + result.Item1 + "," + result.Item2);
            return result.Item1;
        }
    }
